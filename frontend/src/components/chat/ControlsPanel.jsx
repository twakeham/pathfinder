...
export default function ControlsPanel({ value, onChange, compareEnabled, onToggleCompare, compareModel, onChangeCompareModel }) {
  ...
  // Variant B (for comparison): only model selection; parameters reuse primary
  const [bModel, setBModel] = useState(compareModel ?? 'default');
  useEffect(() => { setBModel(compareModel ?? 'default'); }, [compareModel]);
  useEffect(() => { onChangeCompareModel?.(bModel); }, [bModel]);
  ...
  <div className="control">
    ...
    {/* Compare controls (Variant B) */}
        <div className="control">
          ...
          <div className="control-help">Render side-by-side responses. Variant B reuses the primary parameters; you only choose its model.</div>
      ...
      <div className="control-help">Choose a second model to compare against; parameters reuse the primary.</div>
